...
describe('/assets/currencies', () => {
    before(async () => { await request.SESSION(); });

    const baseRoute = '/assets/currencies';
    ...
    const assertId = (body) => {
        ...
        assert.isString(body.result.short);
        assert.isNumber(body.result.exchange);
