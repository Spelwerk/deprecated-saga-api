...
describe('/characteristics/body-parts', () => {
    before(async () => { await request.SESSION(); });

    const baseRoute = '/characteristics/body-parts';
    ...
    const assertRoot = (body) => {
        ...
        _.forEach(body.results, (item) => {
            ...
            assert.isNumber(item.sortOrder);
    ...
    const assertId = (body) => {
        ...
        assert.isNumber(body.result.sortOrder);
        assert.isString(body.result.name);
        assert.isBoolean(body.result.weapon);
