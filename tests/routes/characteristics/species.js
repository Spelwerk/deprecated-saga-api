...
describe('/characteristics/species', () => {
    before(async () => { await request.SESSION(); });

    const baseRoute = '/characteristics/species';
    ...
    const assertId = (body) => {
        ...
        assert.isBoolean(body.result.isPlayable);
        assert.isBoolean(body.result.canMagic);
        assert.isNumber(body.result.maxAge);

        assert.isNumber(body.result.points.multiply);
    ...
    it('/ POST', async () => {
        ...
        const payload = {
            ...
            playable: false,
            manifestation: true,
            max_age: randomNumber(50,200),
            multiply_points: randomNumber(1,3),
        };
