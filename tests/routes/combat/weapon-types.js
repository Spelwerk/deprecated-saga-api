...
describe('/combat/weapon-types', () => {
    before(async () => { await request.SESSION(); });

    const baseRoute = '/combat/weapon-types';
    ...
    const assertId = (body) => {
        ...
        assert.isBoolean(body.result.isAugmentation);
        assert.isBoolean(body.result.isManifestation);
        assert.isBoolean(body.result.isForm);
        assert.isBoolean(body.result.isSpecies);
